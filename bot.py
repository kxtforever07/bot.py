...
def add_points(user_id, points):
    uid = str(user_id)
    if uid not in users:
        users[uid] = {"points": 2, "total_refers": 0}
...
def add_referral(referrer_id):
    ...
    if uid not in users:
        users[uid] = {"points": 2, "total_refers": 0}
    users[uid]["total_refers"] = users[uid].get("total_refers", 0) + 1
    save_users(users)

def get_referrals(user_id):
    return users.get(str(user_id), {}).get("total_refers", 0)
...
async def start(update, context):
    ...
    if args and args[0].startswith("ref_"):
        ...
        if referrer_id != user_id:
            ...
            await context.bot.send_message(referrer_id, f"✅ Someone joined using your refer link! +2 points")
    
    # Init user
    if str(user_id) not in users:
        users[str(user_id)] = {"points": 2, "total_refers": 0}
        save_users(users)
